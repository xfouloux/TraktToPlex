...
if ($isWindows) {
  ...
  if ($env:ARCH -eq "amd64") {
	...
	"$($image):$env:APPVEYOR_REPO_TAG_NAME-linux-arm64-dotnet-$env:dotnet_version" `
	...
	docker manifest annotate "$($image):$env:APPVEYOR_REPO_TAG_NAME" "$($image):$env:APPVEYOR_REPO_TAG_NAME-linux-arm64-dotnet-$env:dotnet_version" --os linux --arch arm --variant v8
